...
message(STATUS "Running 'towncrier build' command to generate release notes...")
execute_process(
    COMMAND ${Python_EXECUTABLE} -m pip show numpy
...
message("")
if (RES_VAR EQUAL 0)
    string(REGEX MATCH "Version: [0-9]+\\.[0-9]+\\.[0-9]+" VERSION_LINE "${OUT_VAR}")
...
message("")
execute_process(
    COMMAND ${Python_EXECUTABLE} -m towncrier build
...
message(STATUS "Running 'python preprocess.py' command to generate 'Doxyfile' file...")
remove_cmake_message_indent()
message("")
execute_process(
    COMMAND ${Python_EXECUTABLE} preprocess.py
...
message("")
restore_cmake_message_indent()
set(DOXYFILE_PATH "build/doxygen/Doxyfile")


message(STATUS "Running 'doxygen' command to genereate .xml files...")
set(DOXYFILE_PATH "build/doxygen/Doxyfile")
remove_cmake_message_indent()
message("")
message("DOXYFILE_PATH      = ${DOXYFILE_PATH}")
message("WORKING_DIRECTORY  = ${PROJ_OUT_REPO_DOCS_DIR}")
message("")
execute_process(
...
message(STATUS "Removing 'version-switcher' from 'html_theme_options.navbar_end' list in 'conf.py' file...")
set(DOCS_CONF_PY_FILE "${PROJ_OUT_REPO_DOCS_CONFIG_DIR}/conf.py")
file(READ "${DOCS_CONF_PY_FILE}" DOCS_CONF_PY_CNT)
set(NAVBAR_END_LIST_REGEX "(\"navbar_end\"[ ]*:[ ]*[\[][^\]]*[\]])")
string(REGEX MATCH "${NAVBAR_END_LIST_REGEX}" OLD_NAVBAR_END_LIST "${DOCS_CONF_PY_CNT}")
if (OLD_NAVBAR_END_LIST AND
    OLD_NAVBAR_END_LIST MATCHES "\"version-switcher\"")
    string(REGEX REPLACE "\"version-switcher\"," "" NEW_NAVBAR_END_LIST "${OLD_NAVBAR_END_LIST}")
    string(REGEX REPLACE "${NAVBAR_END_LIST_REGEX}" "${NEW_NAVBAR_END_LIST}" DOCS_CONF_PY_CNT "${DOCS_CONF_PY_CNT}")
    file(WRITE "${DOCS_CONF_PY_FILE}" "${DOCS_CONF_PY_CNT}")
    remove_cmake_message_indent()
    message("")
    message("Removed 'version-switcher' from 'html_theme_options.navbar_end' list in 'conf.py' file.")
    message("File: ${DOCS_CONF_PY_FILE}")
    message("")
    message("[OLD_NAVBAR_END_LIST_BEGIN]")
    message("${OLD_NAVBAR_END_LIST}")
    message("[OLD_NAVBAR_END_LIST_END]")
    message("[NEW_NAVBAR_END_LIST_BEGIN]")
    message("${NEW_NAVBAR_END_LIST}")
    message("[NEW_NAVBAR_END_LIST_END]")
    message("")
    restore_cmake_message_indent()
else ()
    remove_cmake_message_indent()
    message("")
    message("No need to remove 'version-switcher' from 'html_theme_options.navbar_end' list in 'conf.py' file.")
    message("File: ${DOCS_CONF_PY_FILE}")
    message("")
    restore_cmake_message_indent()
endif()
